...
(deftest bare-deref-is-auto-wrapped-and-live
  ;; @a reads as (deref a) — a list form — so the macro wraps it in its
  ;; own thunk. With an s/atom the deref subscribes, making the bare-@
  ;; Reagent style work, with text-node-level granularity.
  (testing "compile-time: deref child becomes a fn"
    (let [a (satom/atom 1)]
      (is (fn? (nth (h [:p "n: " @a]) 2)))))

  (testing "runtime: DOM text updates in place on swap!/reset!"
    (let [temp (satom/atom 21)]
      (with-mount
        (h [:p "temperature: " @temp "°C"])
        (fn [root]
          (is (= "<p>temperature: 21°C</p>" (.-innerHTML root)))
          (reset! temp 25)
          (is (= "<p>temperature: 25°C</p>" (.-innerHTML root)))
          (swap! temp inc)
          (is (= "<p>temperature: 26°C</p>" (.-innerHTML root))))))))

(deftest prop-deref-is-auto-wrapped-and-live
  (testing "compile-time: {:value @a} becomes {:value (fn [] @a)}"
    (let [a      (satom/atom "x")
          result (h [:input {:value @a}])]
      (is (fn? (:value (second result))))))

  (testing "compile-time: :on* and :ref values are never wrapped"
    (let [handler-atom (satom/atom nil)
          result       (h [:button {:onClick @handler-atom
                                    :ref     @handler-atom}])]
      (is (not (fn? (:onClick (second result)))))
      (is (not (fn? (:ref (second result)))))))

  (testing "runtime: the prop tracks the atom"
    (let [a (satom/atom "hello")]
      (with-mount
        (h [:div {:title @a} "content"])
        (fn [root]
          (let [el (.-firstChild root)]
            (is (= "hello" (.getAttribute el "title")))
            (reset! a "changed")
            (is (= "changed" (.getAttribute el "title")))))))))
