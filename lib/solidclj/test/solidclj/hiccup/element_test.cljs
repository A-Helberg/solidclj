...
(deftest plain-atom-is-not-special
  (testing "child slot: dropped from the DOM, with a dev warning"
    (let [a     (atom "nope")
          html  (volatile! nil)
          warns (with-captured-warns
                  #(vreset! html (render-html [:div a])))]
      (is (= "<div></div>" @html))
      (is (some #(re-find #"plain atom" %) warns))))

  (testing "prop slot: not bridged, with a dev warning"
    (let [a     (atom "v")
          warns (with-captured-warns
