...
(defn create-memo
  "Memoized derived signal: returns a getter that re-computes when
  its tracked reads change and propagates only when `equals` says the
  result changed."
  ([f]        (solid/createMemo f))
  ([f equals] (solid/createMemo f js/undefined #js {:equals equals})))

(defn create-effect
  "Run `f` (0-arg) tracked; re-runs when its reads change. Mapped to
  Solid's createRenderEffect (synchronous), matching the JVM runtime's
  create-effect."
  [f]
  (solid/createRenderEffect (fn [_] (f) nil) nil))

(defn create-root
  "Run `f` under a fresh root owner. Returns [result dispose!] — the
  same contract as the JVM runtime."
  [f]
  (solid/createRoot (fn [dispose] [(f) dispose])))

(defn defer!
  "Run `f` after the current synchronous reactive turn — a microtask
  here; the JVM runtime uses a post-propagation queue. Lets a
  release/resubscribe pair within one re-run cancel out instead of
  tearing shared state down and up."
  [f]
  (js/queueMicrotask f))
