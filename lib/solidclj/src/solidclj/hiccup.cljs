(ns solidclj.hiccup
  ...
  Atoms render live
  =================
  A CLJS atom (or any IDeref + IWatchable thing — `r/atom` works too)
  passed *un-deref'd* into the tree is auto-bridged to a Solid signal:
  the walker mirrors its value through `createSignal`, registers a
  watcher, and tears the watcher down when the owner scope disposes.

      (def time (atom nil))

      [:div time]    ;; live  — re-renders when @time changes
      [:div @time]   ;; static — current value, captured once

  Do NOT `@` deref inside `(fn [] …)` reactive thunks — that bypasses
  the bridge. Pass the atom underef'd into hiccup slots (:when, props,
  :dynamic/:component, children) or use createSignal getters. See
  `docs/atoms-and-reactivity.md`.

  Same goes for prop values: `[:input {:value some-atom}]` is reactive,
  `[:input {:value @some-atom}]` is a one-shot snapshot.
  ...
  ---------------
    ...
    some-atom                       Live read of an atom (live)
    \\@some-atom                    One-shot snapshot
  ...
  (:require [clojure.string]
            ...
            ["solid-js/h"          :as    solid-h :refer [Fragment]]
            ["solid-js/web"        :as    solid-web :refer [Dynamic Portal]]
            [missionary.core        :as    m]))

(declare as-element)

(defn- atom-like?
  "True for things we can subscribe to: anything that's both IDeref and
  IWatchable. Covers `cljs.core/atom`, Reagent's r/atom, and any custom
  watchable holder. Excludes plain IDeref-only things like `delay` /
  `volatile` whose values can't notify on change."
  [x]
...
(defn- atom->signal-getter
  ...
  [a]
  (if (some? (getOwner))
    (let [[get-v set-v] (createSignal @a)
          k             (gensym "solidclj.hiccup/atom-watch-")]
      (add-watch a k (fn [_k _ref _old new] (set-v new)))
...
(defn- make-getter [flow]
  (let [[get-v set-v] (createSignal nil)
        cancel        ((m/reduce (fn [_ v] (set-v v) nil) nil flow)
                       (fn [_] nil)
                       (fn [e] (js/console.error "[solidclj.hiccup] flow error" (str e))))]
    (onCleanup cancel)
    get-v))
...
(defn ?
  ...
  Accepts either:
  - A **missionary flow** — materialises it into a Solid signal and returns the
    signal getter fn. Call the result to read the current value: ((? flow)).
    Safe to call multiple times with the same flow — deduplicated per owner.
  - An **atom-like** (IDeref + IWatchable, e.g. cljs.core/atom, r/atom) —
    bridges it to a Solid signal and returns the current value directly.
    Use inside reactive thunks: (:key (? my-atom)).

  Analogous to m/? in missionary."
  [x]
  (if (atom-like? x)
    (if (some? (getOwner))
      ((atom->signal-getter x))
      (do
        (when ^boolean goog.DEBUG
          (js/console.warn "[solidclj.hiccup] hiccup/? called outside a reactive owner; returning snapshot."))
        @x))
    (if-let [owner (getOwner)]
      (let [cache (or (.get owner-cache owner)
                      (let [m (js/Map.)]
                        (.set owner-cache owner m)
                        (onCleanup #(.delete owner-cache owner))
                        m))]
        (or (.get cache x)
            (let [getter (make-getter x)]
              (.set cache x getter)
              getter)))
      (do
        (when ^boolean goog.DEBUG
          (js/console.warn "[solidclj.hiccup] hiccup/? called outside a reactive owner; returning nil."))
        (fn [] nil)))))
...
(defn- normalize-props
  "Converts a hiccup props map to a JS object suitable for solid-h.
   Applies:
    ...
    - any other atom-valued prop is bridged via atom->thunk so e.g.
      [:input {:value some-atom}] is live."
    ...
    (cond
      ...
      (let [js-comp (first more)
            ...
            [props children] (if (map? (first rest*))
                               ...
                               [nil rest*])]
        (apply solid-h js-comp (normalize-props props [] nil)
      ...
      (let [children (if (map? (first more)) (next more) more)]
        (apply solid-h Fragment nil (walk-children children)))
      ...
      (let [props (first more)
        ...
        (apply solid-h Dynamic js-props (walk-children children)))
      ...
      (let [props (first more)
        ...
        (apply solid-h Portal js-props (walk-children children)))
      ...
      (let [props (first more)
        ...
        (solid-h Index
                 ...
                 (fn [item-getter index]
                   ;; solid-h sometimes returns a thunk that defers
      ...
      (let [[base shorthand-classes shorthand-id] (parse-tag tag)
        ...
        (apply solid-h
               base
               (normalize-props props shorthand-classes shorthand-id)
               (walk-children children)))
      ...
      (solid-h (fn [_props] (as-element (apply tag more))) nil)
...
(defn as-element
  ...
  (cond
    ...
    (atom-like? form) (atom->thunk form)
    (fn? form)            (fn [] (as-element (form)))
    :else                 form))
