(ns solidclj.hiccup
  ...
  Do NOT `@` deref a PLAIN atom inside `(fn [] …)` reactive thunks —
  that bypasses the bridge. Pass the atom underef'd into hiccup slots
  (:when, props, :dynamic/:component, children) or use createSignal
  getters. See `docs/atoms-and-reactivity.md`.

  `solidclj.satom/atom` (exported as `solidclj.api/atom`) lifts this
  restriction: it is a real atom whose deref also subscribes the
  current Solid tracking scope, so `(fn [] @my-satom)` IS live.
